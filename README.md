...
- Local clone of [`mongodb/agent-skills`](https://github.com/mongodb/agent-skills). Tasks resolve eval and skill paths via the `AGENT_SKILLS_DIR` env var; if unset, a sibling `../agent-skills` directory is used. Set it in `.env`, e.g. `AGENT_SKILLS_DIR=/path/to/agent-skills`.
